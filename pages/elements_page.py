...
class TextBoxPage(BasePage):
    ...
    def fill_all_fields(self):  # метод по заполнению всех полей   #send_keys - для вставки в поле
        person_info = next(
            generated_person())  # позволяет делать экземпляр класса с рандомнами значениями только 1 раз!
    ...
    def check_field_form(self):  # проверка получившейся таблицы
        full_name = self.element_is_present(self.locators.CREATED_FULL_NAME).text.split(":")[
            1]  # .split(":")[1]  - чтобы получить 2ю часть таблички, именно со значением
...
class CheckBoxPage(BasePage):
    ...
    def click_random_checkbox(
            self):  # чтобы клики происходили рандомно в чекбоксах, во исполнение 5го принципа тестирования
...
class WebTablePage(BasePage):  # работа с таблицей
    ...
    def add_new_person(self):  # метод по нажатию кнопки добавит ьчеловека, заполнению полей и нажатию ОК
        ...
        while count != 0:
            ...
            return [firstname, lastname, str(age), email, str(salary),
                    department]  # чтобы выводилось в листе и все значения в строках str
    ...
    def check_search_person(
            self):  # проверка найденного в поиске человека, что он есть по итогу в таблице и совпадает. Поиск будет по gjzdkz.otqcz функции удаления
        delete_button = self.element_is_present(self.locators.DELETE_BUTTON)
        row = delete_button.find_element(By.XPATH,
                                         self.locators.ROW_PARENT)  # поиск в родительской строке через функцию удаления
    ...
    def select_up_to_some_rows(self):  # метод выбора количества выводимых строк
        ...
        for x in count:
            ...
            self.element_is_visible((By.CSS_SELECTOR,
                                     f'option[value="{x}"]')).click()  # использую форматирование строки f. На каждой интерации на х будет подставляться то 5, то 10 и тд и сразу кликать
    ...
    def check_new_tab_simple_link(self):
        ...
        if request.status_code == 200:
            ...
            self.driver.switch_to.window(self.driver.window_handles[
                                             1])  # смысл такой: драйвер, переключи свое внимание на окно с индексом 1, и он переключает на новую вкладку, которая окрывается после клика
...
class UploadAndDownloadPage(BasePage):
    ...
    def upload_file(self):
        ...
        text = self.element_is_present(
            self.locators.UPLOADED_RESULT).text  # сохраняю название загруженного на страницу рандомного файла
        return file_name.split("\\")[-1], text.split("\\")[
            -1]  # тут с помощью регулярки я вытаскиваю и возвращаю только последнюю часть названия созданного в моей папке рандомного файла и часть названия, которое отобразилось на странице после загрузки (т.к. их пути не совпадают полностью). Разбиваю по слешам и с помощью [-1] достаю именно послее значение

    def download_file(self):
        link = self.element_is_present(self.locators.DOWNLOAD_FILE).get_attribute(
            "href")  # чтобы скачать картинку, снчала получаю содержимое атрибута href, т.е ссылку c сайта
        ...
        with open(path_name_file, "wb+") as f:
            offset = link_b.find(
                b'\xff\xd8')  # тут я обрезаю скаченную ссылку (уже разбитую на байты) до той части, которая нужна, именно сами полезные байты остаются (без типа, кодировки и тд). Чисто код самого изображения
            f.write(link_b[
                    offset:])  # здесь я записываю ссылку начиная с найденных выше значений (тут и просиходит обрезка, запись без лишнего)
            check_file = os.path.exists(
                path_name_file)  # проверяю что файл, который только создался, что он существует и лежит по этому пути rf"D:\pythonProject\automation_tests_for_portfolio\filetest{random.randint(0,999)}.jpeg"
    ...
    def check_enable_button(self):  # проверка на кликабельность кнопки через 5 сек
        try:
            self.element_is_clickable(
                self.locators.ENABLE_BUTTON)  # тут по умолчанию стоит 5 секунд, прописано в basepage. Поэтому проверяю, что через 5 сек должно все работать, иначе ошибка
    ...
    def check_appear_button(self):  # проверка появления кнопки через 5 сек
        try:
            self.element_is_visible(
                self.locators.VISIBLE_AFTER_BUTTON)  # тут по умолчанию стоит 5 секунд, прописано в basepage
        except TimeoutException:
            return False
        return True
